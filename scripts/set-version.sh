...
if command -v az &>/dev/null && [[ -f "azure-function/infra/main.bicep" ]]; then
  echo "${C_DIM}Compiling azure-function/infra/main.bicep → azuredeploy.json …${C_RST}"
  az bicep build --file azure-function/infra/main.bicep --outfile azure-function/infra/azuredeploy.json
  echo "${C_GRN}✓${C_RST} azure-function/infra/azuredeploy.json recompiled."
  BICEP_COMPILED=true
else
  echo "${C_YLW}⚠${C_RST}  az CLI not found (or main.bicep missing) — azuredeploy.json will be regenerated by CI if needed"
fi
